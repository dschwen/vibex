...
// Denormalize additions into Sub/Neg forms for prettier output.
// Mirrors the RGraph version but operates on AST directly.
inline Expr denormalize_sub(const Expr& e) {
  // Recurse first
  auto rec = [&](const Expr& x) -> Expr { return denormalize_sub(x); };
  if (!e.n) return e;
  ...
  // Only special-case Add trees
  if (auto an = std::dynamic_pointer_cast<AddNode>(e.n)) {
    // Flatten all Add nodes into terms vector
    std::vector<Expr> terms; terms.reserve(4);
    gather_add(e, terms);
    // Classify as positive vs negated, and strip negations
    ...
    if (pos.empty() && !neg.empty()) {
      Expr inner = make_add_local(neg);
      return -inner;
    }

    // Exactly two terms: handle classic patterns Add(a, Neg(b)) -> Sub(a,b)
    if (terms.size() == 2) {
    ...
    // Default: rebuild as sum of children post-denorm
